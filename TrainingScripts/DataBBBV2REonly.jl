...
train_bhps_bulk  = Matrix{Float32}(undef, num_mon, N_train)
train_coeff_bulk = Matrix{Float32}(undef, num_eig, N_train)
train_noise       = Matrix{Float32}(undef, num_mon, N_train)

for i in 1:N_train
    x_true  = randn(Float32, num_eig)
    bhp_vec = solve_bhp(x_true)              # length = num_mon
    bhp32   = Float32.(bhp_vec)

    # Compute 2% noise relative to signal magnitude
    μ  = mean(abs, bhp32)
    σ  = 0.1f0 * max(μ, eps(Float32))
    noise = σ .* randn(Float32, size(bhp32))  # generate noise only

    @inbounds begin
        train_bhps_bulk[:,  i] = bhp32
        train_coeff_bulk[:, i] = x_true
        train_noise[:, i]      = noise      # store only the noise
    end
end
...
#     bhp_vec = add_white_noise(bhp_vec; rel_std=0.02)
#     @inbounds begin
...
@BSON.load "sampled_tails_trainTestval.bson" x_tail h_mon_tail;

train_bhps_rare=Float32.(h_mon_tail[:,1:5000])
train_coeff_rare=Float32.(x_tail[:,1:5000])

train_bhps  = train_bhps_rare
train_coeff = train_coeff_rare 

val_bhps  = h_mon_tail[:,N_train+1:N_train+200]
val_coeff = x_tail[:,N_train+1:N_train+200]
...
function loss_batch(bhps_batch,
    ...
    # total_bhp_loss += sum((bhp_pred .- bhps_batch[:, i]).^2) / num_mon
    # @inbounds for i in 1:B
    #     total_perm_loss += sum((x_pred_mat[:, i] .- coeffs_batch[:, i]).^2) / num_eig
    # end
    # total_perm_loss = sum((x_pred_mat .- coeffs_batch).^2) / num_eig
    total_loss=α_coeff*total_perm_loss
...
batchsize = 5
n_epochs  = 2000  # adjust or use early‐stopping after “patience” of no improvement
...
BSON.@save "DataBBBv2REonlyn10.bson" train_losses val_losses bhp_losses perm_losses
BSON.@save "ModelDataBBBv2REonlyn10.bson" model 
